...
- Для rule-based NLP держать простую модель и не придумывать лишние уровни:
  `sentence -> span -> alias или fact -> signal -> lead_assessment`.
- Низкоуровневых механизмов матчинга ровно три:
  `exact phrase`, `lemmatized phrase`, `alias dictionary`.
- Alias dictionary - это curated словарь именованных сущностей. Он может делать
  нормализацию separator/`ё`/confusable и ограниченный fuzzy, но не
  лемматический поиск.
- Практическое распределение ответственности:
  `alias` отвечает за канонизацию, `fact` отвечает за смысл, `signal`
  отвечает за вывод, `span` и `sentence` дают координаты, `relations` дают
  связность.
- У facts ровно два источника:
  `direct fact rule` или `alias dictionary`.
  Это не dependency facts-on-dictionaries; это два допустимых source
  mechanism для появления факта.
- Лемматический поиск использовать только для смысловых fact rules. Signals
  текстом не владеют и должны зависеть от facts через явные зависимости.
- `operator_noise_fact` считать обычным fact rule. Для него допустимы
  `exact phrase` и `lemmatized phrase`; словарь для него не нужен.
- Один текстовый фрагмент должен иметь одного owner: либо alias, либо fact.
- Если alias-матч найден на span, считать его одним matched alias owner.
  Дополнительный смысл из этого же span должен выходить как derived facts, а не
  как второй/третий competing alias identity на тот же фрагмент.
- Если нужен контекст между фактами, добавлять минимальные relations вроде
  `same_sentence`, `intent_object`, `located_at`, `negates`, а не новые
  абстрактные сущности.
- `same_span` и `same_sentence` считать в первую очередь служебными
  отношениями по координатам. Их лучше вычислять автоматически, а не
  настраивать руками как отдельную бизнес-логику.
...
- Операторская модель rule matching: `exact phrase`, `lemmatized phrase`,
  `alias dictionary`. В UI/API не вводить внутренние Yargy-предикаты вроде
  `caseless`; старые документы в таком формате не поддерживать, а
  пересоздавать или мигрировать в текущую модель данных.
