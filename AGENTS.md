...
- Операторская модель rule matching: только точные фразы и лемматические фразы.
  В UI/API не вводить внутренние Yargy-предикаты вроде `caseless`; старые
  документы в таком формате не поддерживать, а пересоздавать или мигрировать
  в текущую модель данных.
