"""Allow configurable Telegram raw export jobs.

Revision ID: 0025_export_telegram_raw_job
Revises: 0024_telegram_raw_export_runs
...
revision: str = "0025_export_telegram_raw_job"
down_revision: str | None = "0024_telegram_raw_export_runs"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_JOB_TYPES_WITH_RAW_EXPORT = (
...
def _scheduler_jobs_allow_raw_export(bind) -> bool:  # noqa: ANN001
    sql = bind.execute(
        sa.text("select sql from sqlite_master where type='table' and name='scheduler_jobs'")
    ).scalar_one()
    return "export_telegram_raw" in str(sql)
