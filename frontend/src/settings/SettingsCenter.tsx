...
import type {
  ...
  TelegramSourceChatSettings,
  TelegramUserbotAccountSettings
} from "./types";
...
export function SettingsTargetDialog({
  target,
  settings,
  loading,
  error,
...
function SettingLink({ target, children }: { target: SettingsTarget | null; children: ReactNode }) {
  if (!target) {
    return <>{children}</>;
  }
...
export function settingsSectionForTarget(target: SettingsTarget): SettingsSection {
  if (target.kind === "signal") {
    return "signals";
  }
  if (target.kind === "fact") {
