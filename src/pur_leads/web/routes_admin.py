...
def _setting_help(key: str) -> dict[str, str]:
    explicit: dict[str, tuple[str, str, str]] = {
        ...
        "interest_core_llm_enhancement_parallelism": (
            "Ядро интересов",
            "Сколько LLM-фрагментов кандидатов ядра обрабатывать одновременно.",
            "0 означает auto по эффективному лимиту выбранной модели; 1 — строго последовательно; N ускоряет обработку, но реестр canonical_name обновляется между волнами.",
        ),
        "interest_core_llm_enhancement_chunk_max_attempts": (
            "Ядро интересов",
            "Сколько раз повторять один LLM-фрагмент, если модель вернула невалидный JSON.",
            "После исчерпания попыток фрагмент помечается ошибкой, а весь прогон продолжает следующие фрагменты.",
        ),
